...
*/
static char *DNSSERVER_parse_dns_name(char *raw_name, char *parsed_name,
...
static int DNSSERVER_parse_request(char *req, size_t req_len, char *dns_reply,
  ...
  for (int i = 0; i < qd_count; i++) {
    char *name_end_ptr =
        DNSSERVER_parse_dns_name(cur_qd_ptr, name, sizeof(name));
    ...
    if (qd_type == QD_TYPE_A) {
      ...
      esp_netif_get_ip_info(esp_netif_get_handle_from_ifkey("WIFI_AP_DEF"),
                            &ip_info);
      ESP_LOGD(TAG, "Answer with PTR offset: 0x%X and IP 0x%X",
...
*/
void DNSSERVER_task(void *pvParameters) {
  char rx_buffer[128];
  char addr_str[128];
  int addr_family;
  int ip_protocol;
  ...
  while (1) {
    ...
    while (1) {
      ...
      // Data received
        ...
        int reply_len =
            DNSSERVER_parse_request(rx_buffer, len, reply, DNS_MAX_LEN);

        // ESP_LOGI(TAG, "Received %d bytes from %s | DNS reply with len: %d",
        // len,
        //          addr_str, reply_len);
        if (reply_len <= 0) {
...
void DNSSERVER_start(void) {
  xTaskCreate(DNSSERVER_task, "dns_server", 4096, NULL, 5, NULL);
}